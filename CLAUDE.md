...
**`arbor-scip`** — SCIP index ingestion, for compiler-accurate graphs. Tree-sitter cannot resolve `obj.method()` without type inference; a [SCIP](https://github.com/scip-code/scip) index carries the compiler's own resolution instead. **Language-neutral**: the SCIP grammar does not vary by producer, so one code path reads `scip-java` (Java, Kotlin), `scip-typescript`, `scip-python`, `rust-analyzer scip`, `scip-clang`, `scip-dotnet`, `scip-go`, `scip-ruby`, `scip-php` and `scip-dart`. Only `scip-java` is *invoked* by Arbor (`arbor scip --background`) and only it is verified against a real index; for the rest, run the indexer and pass its `index.scip`. Modules:
